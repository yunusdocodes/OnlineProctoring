...
TestResult,Question,UserResponse,UserProfile,AdminSettings,RecentActivity,Announcement, Test,
...
@parser_classes([MultiPartParser, FormParser, JSONParser])  # Handles file and JSON
def log_malpractice(request):
    try:
        if "frame" in request.FILES:
            # Case: Frame image sent (from webcam)
            frame = request.FILES['frame']
            email = request.user.email  # fallback
            test_id = request.data.get("test_id")
            event_type = "Object detected in frame"

            if not test_id:
                return Response({"error": "Missing test_id in request"}, status=400)

            test = Test.objects.get(id=test_id)

            log = MalpracticeLog.objects.create(
                email=email,
                test=test,
                event_type=event_type,
                frame=frame  # ✅ Let Django handle the save
            )

            return Response({
                "message": "Frame saved and malpractice logged",
                "frame_url": log.frame.url,  # ✅ Optional: return the frame URL
                "id": log.id
            }, status=201)

        else:
            # Case: JSON malpractice event
            data = request.data
            email = data.get("email")
            test_id = data.get("test_id")
            event_type = data.get("event_type")

            if not email or not test_id or not event_type:
            ...
            return Response({"message": "Malpractice logged", "id": log.id}, status=201)
