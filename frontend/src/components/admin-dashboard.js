...
<IconButton color="inherit" onClick={handleOpenNotifications}>
  ...
  {[
    ...
    },
    {
      label: "Top Scorers",
        value:
          dashboardData?.top_scorers && dashboardData.top_scorers.length > 0
            ? dashboardData.top_scorers
                .map(
                  scorer =>
                    `${scorer.name || "Unknown"} (${
                      scorer.average_score != null
                        ? scorer.average_score.toFixed(2)
                        : "N/A"
                    })`
                )
                .join(", ")
            ...
            if (elements.length) {
              ...
              chart.data.datasets.forEach((dataset) => {
                ...
                const originalColors = Array.isArray(dataset.backgroundColor)
                  ...
                  : new Array(chart.data.labels.length).fill(dataset.backgroundColor);

                // Only change if the color is not already the hover color
                if (originalColors[index] !== 'rgba(0, 51, 102, 1)') {
                  originalColors[index] = 'rgba(0, 51, 102, 1)';
