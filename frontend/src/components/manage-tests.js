...
import {Leaderboard, MoreVert, Edit, ContentCopy, Delete, } from "@mui/icons-material";
...
const ManageTestsPage = () => {
  ...
  const handleTestResult= ()=>{
    if (selectedTest?.id) {
      navigate(`/test/${selectedTest.id}/scores`);
      handleMenuClose();
    } else {
      ...
      <Box sx={{ display: "flex" }}>
        <Box component="main" sx={{ flexGrow: 1, p: 3, pt: 10 }}>
          ...
          <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={handleMenuClose}>
            ...
            <MenuItem onClick={handleTestResult}>
              <Leaderboard fontSize="small" sx={{ mr: 1 }} /> View test result
            </MenuItem>
          </Menu>
