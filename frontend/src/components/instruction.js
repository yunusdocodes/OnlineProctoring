        ...
        if (uuid) {
          axios.get(process.env.REACT_APP_API_BASE_URL+`/decode-test-uuid/${uuid}/`)
            .then(res => {
              ...
              return fetch(process.env.REACT_APP_API_BASE_URL+`/tests/${decodedId}/`, {
                method: "GET",
                headers: {
...
if (validDetections.length > 1) throw new Error('Multiple faces detected');
    ...
    const sendToBackend = async (imageBlob, descriptor) => {
        ...
        try {
            const response = await fetch(process.env.REACT_APP_API_BASE_URL+'/capture/', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    const handleStartTest = () => {
        ...
        fetch(process.env.REACT_APP_API_BASE_URL+`/tests/${testId}/save_consent/`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Token ${userToken}`
